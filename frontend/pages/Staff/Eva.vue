                ...
                <v-card>
                    ...
                    <v-card-text>
                        <v-form @submit.prevent="saveMember">
                            <v-row>
                                <v-col cols="12" md="6">
                                    <v-select label="ผู้รับการประเมิน" :items="member.map(p => ({title:`${p.first_name} ${p.last_name}`,value:p.id_member}))" v-model="form.id_member" :error-messages="error.id_member" />
                                </v-col>
                                <v-col cols="12" md="6">
                                    <v-text-field label="วันที่ออกแบบประเมิน" v-model="form.day_eva" type="date" :error-messages="error.day_eva" />
                                </v-col>
                                 <v-col cols="12" md="12">
                                    <v-select :items="round.map(p => ({title:`รอบการประเมิน ${p.round_sys} ปี ${p.year_sys}`,value:p.id_sys}))" label="รอบการประเมิน" v-model="form.id_sys" :error-messages="error.id_sys" />
                                </v-col>
                                <v-row>
                                    <v-col cols="12" md="6">
                                        <v-btn color="blue" type="submit" block>{{ form.id_eva ? 'อัปเดต' : 'บันทึก' }}</v-btn>
                                    </v-col>
                                    <v-col cols="12" md="6">
